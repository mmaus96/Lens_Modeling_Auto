...
from Lens_Modeling_Auto.fit_sequence_functions import model_deblended_alt
...
# nohup python -u ./Lens_Modeling_Auto/model_deblended.py > CFIS_deblended/deblended_image_2/modeling_results_deblended/results_deblended_lenslight/output.log &
...
im_path = data_path + 'originals/lenses'
...
# select_objects = ['12','21','24','26','29','92','99','106','','','','','','','','','','','','','','']
...
for i,obj in enumerate(obj_names):
    for x in im_files:
        if obj == re.findall('\d+', x)[obj_name_location]: im = x
    ...
    data_pairs_dicts.append({'image_data': im , 'LRG_data': LRG,'source_data': source,'psf': psf , 'noise_map': noise, 
...
#                                 ,['MCMC', {'n_burn': 0, 'n_run': 50, 'walkerRatio': 10, 'sigma_scale': .1,'threadCount':numCores}]
        ...
        fitting_kwargs_list = [['PSO', {'sigma_scale': 1, 'n_particles': 500, 'n_iterations': 2000,'threadCount': numCores}]
...
#      multi_band_list, kwargs_constraints) = model_deblended_alt(fitting_kwargs_list,lens_model_list,
#                                                         source_model_list,lens_light_model_list,
#                                                         lens_initial_params,source_initial_params,
#                                                         lens_light_initial_params,kwargs_data,
#                                                         kwargs_data_LRG,kwargs_data_source,kwargs_psf,mask_list)
    (kwargs_params,kwargs_fixed, kwargs_result,
     chain_list,kwargs_likelihood, kwargs_model, kwargs_data_joint, 
...
#                                 ,['MCMC', {'n_burn': 0, 'n_run': 50, 'walkerRatio': 10, 'sigma_scale': .05,'threadCount':numCores}]
    ...
    (chain_list, kwargs_result,kwargs_params,kwargs_likelihood, 
     kwargs_model, kwargs_data_joint,
     multi_band_list, kwargs_constraints) = full_sampling(fitting_kwargs_list,kwargs_params, 
                                                          kwargs_data, kwargs_psf,lens_model_list, 
                                                          source_model_list,lens_light_model_list,mask_list)
