...
#                 with open(mask_pickle_path + '{}/Image_{}-{}.pickle'.format(band_list[k],it+1,
...
#                               ]
    ...
    if fix_seed:
#         with open(source_seed_path + 'Image_{}-{}.pickle'.format(it+1,data_pairs_dicts[it]['object_ID']), 'rb') as handle:
    ...
    (lens_initial_params,
     ...
     ps_initial_params) = initial_model_params(lens_model_list= lens_model_list,
...
#     exec(open('Lens_Modeling_Auto/initial_modeling_fit.py').read())
    ...
    for i in range(len(kwargs_data)):
        ...
        if point_source_model_list !=None:
            multi_ps_model_list.extend(deepcopy(point_source_model_list))
...
#     exec(open('Lens_Modeling_Auto/save_to_csv_full_old.py').read())
    ...
    if ((red_X_squared >= 1.0) and (use_shapelets == True)):
        n_max = 6
        print('\n')
        ...
        lens_params_update = deepcopy(kwargs_params['lens_model'])        
        lens_light_params_update = deepcopy(kwargs_params['lens_light_model'])
