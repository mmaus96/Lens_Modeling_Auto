...
# nohup python -u ./Lens_Modeling_Auto/rings_modeling_script.py > Rings_catalog/new_priors/results_Jun11/output_logs/output160_180.log &
...
for i,obj in enumerate(obj_names):
    ...
    if ra_dec == 'csv':
        ...
        for j in range(len(df_info.loc[:,:])):
            if float(df_info.loc[j,id_col_name]) == float(obj): RA,DEC = df_info.loc[j,'ra_y'],df_info.loc[j,'dec_y']
    else: RA, DEC = 'N/A','N/A'
...
for it in range(len(data_pairs_dicts[530:])):    
    it += 530
...
#                                 ,['MCMC', {'n_burn': 0, 'n_run': 50, 'walkerRatio': 10, 'sigma_scale': .1,'threadCount':numCores}]
    ...
    name = 'Image_{}-{}.pickle'.format(it+1, data_pairs_dicts[it]['object_ID'])
